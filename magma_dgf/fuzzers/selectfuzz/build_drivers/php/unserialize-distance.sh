...
for fuzzerName in $FUZZERS; do
        ...
        for bcFile in sapi/fuzzer/$fuzzerName.*.bc; do
           baseName=$(basename "$bcFile")
           newName=${baseName/#php-fuzz-/}
           # Copy the .bc file to $OUT with the new name
           cp "$bcFile" "$OUT/$newName"
        done
done
